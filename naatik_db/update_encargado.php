<?php
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE Encargado set Email = ?, Nombre = ?, Telefono =? WHERE Email = ?";
			$q = $pdo->prepare($sql);
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * FROM Encargado where Email = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($id));
	...
	<body>
    	<div class="container">
    		<div class="span10 offset1">
    			<div class="row">
		    		<h3>Actualizar datos de un encargado</h3>
		    	</div>

	    			<form class="form-horizontal" action="update_encargado.php?id=<?php echo $id?>" method="post">
					  ...
					  <div class="form-actions">
						  ...
						  <a class="btn" href="Encargados.php">Regresar</a>
						</div>
					</form>
				</div>
